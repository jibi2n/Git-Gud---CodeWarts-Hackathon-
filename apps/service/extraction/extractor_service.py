        ...
        for comp in self.catalog:
            ...
            if self._passes_gate(cid, t):
                if cid == "w-01":
                    ...
                    out.append({"id": cid, "taglish_label": "Pagwe-welding ng plates", "english_label": label, "confidence": 0.65, "evidence_span": ev})
                if cid == "w-02":
                    ev = next((s for s in segs if "safety" in s.lower() or "ppe" in s.lower() or "mask" in s.lower()), segs[0] if segs else None)
                    out.append({"id": cid, "taglish_label": "Safety practices sa welding", "english_label": label, "confidence": 0.6, "evidence_span": ev})
                if cid == "w-03":
                    ev = next((s for s in segs if "drawing" in s.lower() or "sketch" in s.lower() or "blueprint" in s.lower()), segs[0] if segs else None)
                    out.append({"id": cid, "taglish_label": "Pagbasa ng drawings/sketch", "english_label": label, "confidence": 0.6, "evidence_span": ev})
                if cid == "w-04":
                    ev = next((s for s in segs if "prepare" in s.lower() or "materials" in s.lower() or "tools" in s.lower()), segs[0] if segs else None)
                    out.append({"id": cid, "taglish_label": "Pag-prepare ng welding materials", "english_label": label, "confidence": 0.58, "evidence_span": ev})
